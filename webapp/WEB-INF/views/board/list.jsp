		...
		<div id="content">
			<div id="board">
				...
				<table class="tbl-ex">
					...
					<tr>
						<td>${vo.rn }</td>
						<td><a href="${pageContext.request.contextPath }/board/read?flag=1&hit=${vo.hit }&no=${vo.no }&currNo=${page.currNo}&kwd=${param.kwd}">${vo.title}</a></td>
						<td>${vo.name }</td>
						<td>${vo.hit }</td>
						<td>${vo.regDate }</td>
						...
						<td>
							...
							<a href="${pageContext.request.contextPath }/board/delete?no=${vo.no }&currNo=${page.currNo}" class="del">삭제</a>
							</c:if>
						</td>
				...
				<div class="pager">
					<ul>
						...
						<c:if test="${!(empty param.kwd) }">
							<li><a href="${pageContext.request.contextPath }/board/search?currNo=${page.currNo-1}&kwd=${param.kwd}">◀</a></li>
						</c:if>
						...
						<c:forEach var="i" begin="${page.firstNo }" end="${page.endNo }" step="1">
							<c:if test="${i != page.currNo }">
								...
								<c:if test="${!(empty param.kwd) }">
									<li><a href="${pageContext.request.contextPath }/board/search?currNo=${i }&kwd=${param.kwd}">${i }</a></li>
								</c:if>
